...
result_file = "./res_t_4_7_10_max.txt"
...
for x in range(0,5): 
    for proc in range(1,13):
        proces = subprocess.Popen(["bash", "-c", "export OMP_NUM_THREADS="+str(proc)+"; ./bucket " + str(size) + " 32766 "+ str(bucket_size)+ " " + str(proc) + " " + result_file])
        proces.wait()
        print(proces.stdout)
